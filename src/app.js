(function() {
  ...
  function FormCtrl(Data, $scope) {
    ...
    $scope.$watch(
      angular.bind(vm.form_data, function() {
  ...
  function Data() {
    ...
    function save(musician) {
      if (musician.id) {
        // existing record so update the model
        var idx = musician.idx;
        service.musicians[idx] = musician;
      } else {
        // new record so add to the model
        var id;
        Math.max.apply(
          Math,
          service.musicians.map(function(o) {
